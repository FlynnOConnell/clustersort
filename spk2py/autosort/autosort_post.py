...
del_old = True  # If files have been worked on but not saved, delete old unitsJosh
if len(Analyzedlist) > 0:  # If there are any json files in the analyzed folder
    for n in range(len(Analyzedlist)):
        if n == 0:  # If the first file
            ...
            var = 6
if reanalyze == 1:
    del_old = easygui.ynbox(
        msg='Do you want to delete old cells in the files you have already done? The files are: \n\n' + FilesAnalyzed)
    if not del_old:
        redo = easygui.ynbox(
            msg='Do you want to add new cells to the files you have already done?')  # Ask if they want to add any cells
elif reanalyze == 0:
    del_old = False
    redo = False
...
for files in file_list:
    file_iso = pd.DataFrame(
        columns=['IsoRating', 'File', 'Channel', 'Solution', 'Cluster', 'wf count', 'ISIs (%)', 'L-Ratio',
                 'Post-Process Date', 'Recording Type'])
    ...
    if files[-2:] == 'h5':  # If a specific file is an .h5 file
        ...
        if not redo:  # If user did not want to redo the already done files
        ...
        if first:  # If on the first file
            ...
            check = easygui.ynbox(
                msg='Do you want to continue onto the next file? The file is %s?' % hdf5_name)  # Ask user if they want to keep going
        if not check:  # If they do not want to continue
            break  # Exit program
        ...
        # Make a table under /sorted_units describing the sorted units. If unit_descriptor already exists, just open
        # it up in the variable table
        try:
        ...
        while True:
            try:
                ...
                electrode_num = easygui.multenterbox(
                    msg='For file %s, which electrode (1-%s) do you want to choose? \nHit cancel to move to the next '
                        'file' % (
                            hdf5_name[:-3], str(elNum)), fields=['Electrode #'])
                ...
                num_clusters = easygui.multenterbox(
                    msg='Which solution do you want to choose for electrode %i?' % (electrode_num + 1),
                    fields=['Number of clusters in the solution'])
                num_clusters = int(num_clusters[0])

                # Get hpc_cluster choices from the chosen solution
                clusters = easygui.multchoicebox(
                    msg='For file %s, electrode %i, in the %i hpc_cluster batch, Which clusters do you want to choose?' % (
                        hdf5_name[:-3], electrode_num + 1, num_clusters),
                    choices=tuple([str(i) for i in range(num_clusters)]))

                # Check if the user wants to merge clusters if more than 1 hpc_cluster was chosen. Else ask if the user
                # wants to split/re-hpc_cluster the chosen hpc_cluster
                merge = False
                re_cluster = False
                if len(clusters) > 1:
                    ...
                    re_cluster = str(easygui.ynbox(msg='Do you want to split this hpc_cluster?'))
                    re_cluster = ast.literal_eval(re_cluster)

                # Load data from the chosen electrode and solution
                spike_waveforms = np.load(
                    './' + hdf5_name[:-3] + '/spike_waveforms/electrode %i/spike_waveforms.npy' % (electrode_num + 1))
                spike_times = np.load(
                    './' + hdf5_name[:-3] + '/spike_times/electrode %i/spike_times.npy' % (electrode_num + 1))
                energy = np.load(
                    './' + hdf5_name[:-3] + '/spike_waveforms/electrode %i/energy.npy' % (electrode_num + 1))
                amplitudes = np.load(
                    './' + hdf5_name[:-3] + '/spike_waveforms/electrode %i/spike_amplitudes.npy' % (electrode_num + 1))
                predictions = np.load(
                    './' + hdf5_name[:-3] + '/clustering_results/electrode %i/clusters%i/predictions.npy' % (
                        (electrode_num + 1), num_clusters))
                ...
                # If the user asked to split/re-hpc_cluster, ask them for the clustering parameters and perform clustering
                split_predictions = []
                chosen_split = 0
                if re_cluster:
                    ...
                    if simple_GMM == 0:
                        clustering_params = easygui.multenterbox(
                            msg='Fill in the parameters for re-clustering (using a GMM)',
                        ...
                        n_clusters = int(easygui.multenterbox(
                            msg='Enter a number of target clusters (using a GMM) \n\nNote: You have selected easy GMM \nMaximum Iterations=1000 \nConvergence Criterion=.0001 \nNumber of random restarts=10',
                            fields=['Number of clusters'])[0])
                        n_iter = 1000
                        thresh = .0001
                        n_restarts = 10
                    ...
                    # Cluster the data
                    g = GaussianMixture(n_components=n_clusters, covariance_type='full', tol=thresh, max_iter=n_iter,
                                        n_init=n_restarts)
                    g.fit(data)

                    # Show the hpc_cluster plots if the solution converged
                    if g.converged_:
                        ...
                        for cluster in range(n_clusters):
                            ...
                            # plt.figure(hpc_cluster)
                            slices_dejittered = spike_waveforms[this_cluster,
                                                :]  # Waveforms and times from the chosen hpc_cluster
                            times_dejittered = spike_times[this_cluster]
                            times_dejittered = times_dejittered[
                                split_points]  # Waveforms and times from the chosen split of the chosen hpc_cluster
                            ISIs = np.ediff1d(np.sort(
                                times_dejittered)) / 40.0  # Get number of points between waveforms and divide by frequency per ms (40)
                            ...
                            fig, ax = autosort.Pl2_waveforms_datashader.waveforms_datashader(
                                slices_dejittered[split_points, :], x, dir_name=dsdir)
                            # plt.plot(x-15, slices_dejittered[split_points, :].T, linewidth = 0.01, color = 'red')
                            ax.set_xlabel('Sample (40 points per ms)')
                            ax.set_ylabel('Voltage (microvolts)')
                            ax.set_title("Split Cluster{:d}, 2ms ISI violations={:.1f} percent".format(cluster,
                                                                                                       violations2) + "\n" + "1ms ISI violations={:.1f}%, Number of waveforms={:d}".format(
                                violations1, split_points.shape[0]))
                    else:
                        print(
                    ...
                    chosen_split = easygui.multchoicebox(
                        msg='Which split hpc_cluster(s) do you want to choose? Hit cancel to return to electrode selection.',
                    ...
                    try:
                        ...
                        for choice in chosen_split:
                            ...
                            clcheck = easygui.ynbox(
                                msg='Please verify that this hpc_cluster is correct.\nL-Ratio: {}'.format(
                                    round(Lrats[int(choice)], 3)), image=figname)
                            if clcheck:
                                pass
                            else:
                                easygui.msgbox(
                                    'You indicated that a split hpc_cluster was not correct. Results have not been saved.')
                                continue
                    except:
                        continue
                ...
                if re_cluster:
                    ...
                    for cluster in range(len(chosen_split)):
                        ...
                        unit_waveforms = spike_waveforms[np.where(predictions == int(clusters[0]))[0],
                                         :]  # Waveforms of originally chosen hpc_cluster
                        unit_waveforms = unit_waveforms[
                                         np.where(split_predictions == int(chosen_split[int(cluster)]))[0],
                                         :]  # Subsetting this set of waveforms to include only the chosen split
                        unit_times = spike_times[
                            np.where(predictions == int(clusters[0]))[0]]  # Do the same thing for the spike times
                        ...
                        # single_unit = str(easygui.ynbox(msg = 'Are you mostly-SURE that %s is a beautiful single unit?' % chosen_split[hpc_cluster]))
                ...
                # If only 1 hpc_cluster was chosen (and it wasn't split), add that as a new unit in /sorted_units. Ask if the isolated unit is an almost-SURE single unit
                elif len(clusters) == 1:
                    unit_waveforms = spike_waveforms[np.where(predictions == int(clusters[0]))[0], :]
                    ...
                    ax.set_ylabel('Voltage (microvolts)')
                    ax.set_title(
                        "Channel: " + str(electrode_num + 1) + ", Solution: " + str(num_clusters) + ", Cluster: " + str(
                            clusters[0]))
                    fig.savefig(figname, dpi=image_size)
                    iso = pd.read_csv(hdf5_name[:-3] + '/clustering_results/electrode {}/clusters{}/isoinfo.csv'.format(
                        electrode_num + 1, num_clusters))
                    unit_verify = easygui.ynbox(msg="Please verify that this is the correct unit.\nL-Ratio: {}".format(
                        iso['L-Ratio'][int(clusters[0])]), image=figname)
                    if unit_verify:
                        ...
                        easygui.msgbox(
                            'You indicated that this hpc_cluster was incorrect. Results for this channel have not been saved.')
                        del unit_waveforms
                        continue
                    ...
                    if merge:
                        ...
                        merge_predictions = np.array(
                            [int(clusters[0]) if str(x) in clusters else x for x in predictions])
                        Lrats = clust.get_Lratios(data, merge_predictions)

                        # Show the merged hpc_cluster to the user, and ask if they still want to merge
                        x = np.arange(len(unit_waveforms[0]) / 10) + 1
                        fig, ax = autosort.Pl2_waveforms_datashader.waveforms_datashader(unit_waveforms, x,
                                                                                         dir_name=dsdir)
                        ...
                        ISIs = np.ediff1d(np.sort(
                            unit_times)) / 40.0  # Get number of points between waveforms and divide by frequency per ms (40)
                        violations1 = 100.0 * float(np.sum(ISIs < 1.0) / len(unit_times))
                        violations2 = 100.0 * float(np.sum(ISIs < 2.0) / len(unit_times))
                        proceed = easygui.ynbox(image=figname,
                                                msg='The merged hpc_cluster has %.1f percent (<2ms) and %.1f percent (<1ms) ISI violations out of %i total waveforms. Do you still want to merge these clusters into one unit?' % (
                                                    violations2, violations1,
                                                    len(unit_times)) + '\nL-ratio of merged clusters:{}'.format(
                                                    round(Lrats[int(clusters[0])], 3)))
                        ...
                        if proceed:
                            ...
                            easygui.msgbox(
                                'You indicated that the merged hpc_cluster was not isolated, results for this channel have not been saved.')
                            continue

                    # Otherwise include each hpc_cluster as a separate unit
                    else:
                        bad_cluster = False
                        for cluster in clusters:
                            unit_waveforms = spike_waveforms[np.where(predictions == int(cluster))[0], :]
                            ...
                            ax.set_xlabel('Sample (40 points per ms)')
                            ax.set_ylabel('Voltage (microvolts)')
                            ax.set_title("Channel: " + str(electrode_num + 1) + ", Solution: " + str(
                                num_clusters) + ", Cluster: " + str(cluster))
                            fig.savefig(figname, dpi=image_size)
                            iso = pd.read_csv(
                                hdf5_name[:-3] + '/clustering_results/electrode {}/clusters{}/isoinfo.csv'.format(
                                    electrode_num + 1, num_clusters))
                            unit_verify = str(easygui.ynbox(
                                msg="Please verify that this is the correct unit.\nL-Ratio: {}".format(
                                    iso['L-Ratio'][int(cluster)]), image=figname))
                            if ast.literal_eval(unit_verify):
                                pass
                            else:
                                bad_cluster = True
                                break
                        ...
                        if bad_cluster == False:
                            for cluster in clusters:
                                ...
                                unit_waveforms = spike_waveforms[np.where(predictions == int(cluster))[0], :]
                                unit_times = spike_times[np.where(predictions == int(cluster))[0]]
                                waveforms = hf5.create_array('/sorted_units/%s' % unit_name, 'waveforms',
                                                             unit_waveforms)
                                times = hf5.create_array('/sorted_units/%s' % unit_name, 'times', unit_times)
                                channel = hf5.create_array('/sorted_units/%s' % unit_name, 'channel', unit_chan)
                                unit_description['electrode_number'] = electrode_num
                            ...
                            easygui.msgbox(
                                "You indicated that one of the clusters was incorrect. Results for this channel have not been saved.")
                            continue
            except Exception as e:
                easygui.msgbox(
                    "An error occured, you will be returned to electrode selection\n\nError message:" + str(e))
        ...
        for n in range(len(evtNames)):
            ...
            evtNames[n] = str.split(tempName0[n][2], '(')[0][
                          :-1]  # get event names #change to work with spaces in tastant names
        ...
        try:
            infofile = [file for file in os.listdir(filedir + '/' + os.path.splitext(hdf5_name)[0]) if
                        file.endswith('.info')]
            if len(infofile) > 1:
                sys.exit('Why do you have more than one infofile in this sorted folder?')
            elif len(infofile) == 0:
                sys.exit(
                    "If you don't have an infofile, you should be using an older version of the postprocess script")
            else:
                file_iso['Recording Type'] = config.rec_info(
                    filedir + '/' + os.path.splitext(hdf5_name)[0] + '/' + infofile[0])
                shutil.copy(filedir + '/' + os.path.splitext(hdf5_name)[0] + '/' + infofile[0],
                            'R:/Autobots Roll Out/' + UserName + '/Info_Files')
        except Exception as e:
            print(e)
