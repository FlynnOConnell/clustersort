...
def infofile(pl2_filename, path, sort_time, AS_file, params):
    ...
    with tables.open_file(path + '.h5', 'r') as hf5:
        if hf5.root.__contains__('/SPKC'):
            record_type = 'Continuous Signal'
        else:
            record_type = 'Thresholded Waveforms'
    config = configparser.ConfigParser()
    config['METADATA'] = {'Pl2 File': pl2_filename, 'Recording Type': record_type, 'Run Time': sort_time,
...
def pl2_to_h5(file, filedir, min_licks=1000):
    ...
    def getSPKC(dig, name):
        spkcNames = list(
            set(a.name for a in adinfo if (a.name[:dig] == name)))  # get names of continuous data and add to set list
    ...
    def NotEnoughLicks():
        ...
        f.write(
            "The rat did not lick enough and so cell sorting is irrelevant.")  # Notify that no continuous data was recorded
...
def Processing(electrode_num, pl2_fullpath, params):  # Define function
        ...
        try:
            ...
            if hf5.root.__contains__('/SPKC'):  # if continuous data was recorded
                ...
                plt.plot((recording_cutoff, recording_cutoff),
                         (np.min(np.mean(test_el, axis=1)), np.max(np.mean(test_el, axis=1))), 'k-', linewidth=4.0)
                ...
                fig.savefig(os.path.normpath(hdf5_name[:-3] + '/Plots/{}/cutoff_time.png'.format(electrode_num + 1)),
                            bbox_inches='tight')
                plt.close("all")
                ...
                if len(filt_el) == 0:
                    ...
                    slices, spike_times = clust.extract_waveforms(filt_el, spike_snapshot=[spike_snapshot_before,
                                                                                           spike_snapshot_after],
                                                                  sampling_rate=sampling_rate, STD=STD,
                ...
                if len(slices) == 0 or len(spike_times) == 0:
                    with open(hdf5_name[:-3] + '/Plots/' + str(electrode_num + 1) + '/' + 'no_spikes.txt', 'w') as txt:
                        ...
                        with open(hdf5_name[:-3] + '/clustering_results/electrode {}'.format(
                                electrode_num + 1) + '/success.txt', 'w+') as f:
                ...
                times_final = np.array(getattr(hf5.root.SPKwf, 'SPK' + f'{electrode_num + 1:02d}' + 'times')[
                                       :]) * sampling_rate  # get spike times
            ...
            np.save(os.path.normpath(
                hdf5_name[:-3] + '/spike_waveforms/electrode {}/spike_waveforms.npy'.format(electrode_num + 1)),
                    slices_final)
            np.save(os.path.normpath(
                hdf5_name[:-3] + '/spike_times/electrode {}/spike_times.npy'.format(electrode_num + 1)), times_final)
            ...
            np.save(os.path.normpath(
                hdf5_name[:-3] + '/spike_waveforms/electrode {}/pca_waveforms.npy'.format(electrode_num + 1)),
                    pca_slices)
            np.save(
                os.path.normpath(hdf5_name[:-3] + '/spike_waveforms/electrode {}/energy.npy'.format(electrode_num + 1)),
                energy)
            np.save(os.path.normpath(
                hdf5_name[:-3] + '/spike_waveforms/electrode {}/spike_amplitudes.npy'.format(electrode_num + 1)),
                    amplitudes)
            ...
            fig.savefig(os.path.normpath(hdf5_name[:-3] + '/Plots/{}/pca_variance.png'.format(electrode_num + 1)),
                        bbox_inches='tight')
            plt.close("all")
            ...
            if retried == 1:
                ...
                return
            warnings.warn(
                "Warning, could not allocate memory for electrode {}. This program will wait and try again in a bit.".format(
                    electrode_num + 1))
            retried = 1
            time.sleep(1200)
        except:
    ...
    for i in range(max_clusters - 2):
        ...
        if np.any([len(np.where(predictions[:] == cluster)[0]) <= n_pc + 2 for cluster in range(i + 3)]):
            ...
            with open(hdf5_name[:-3] + '/Plots/%i/%i_clusters' % ((electrode_num + 1), i + 3) + '/invalid_sort.txt',
                      "w+") as f:
            ...
            (electrode_num + 1), i + 3) + '/invalid_sort.txt', "w+") as f:
                f.write("There are too few waveforms to properly sort this clustering")
            continue
        ...
        for cluster in range(i + 3):
            ...
            reject_wf = \
            np.where(cluster_amplitudes <= cluster_amplitude_mean - wf_amplitude_sd_cutoff * cluster_amplitude_sd)[0]
        ...
        np.save(os.path.normpath(
            hdf5_name[:-3] + '/clustering_results/electrode {}/clusters{}/predictions.npy'.format(electrode_num + 1,
                                                                                                  i + 3)), predictions)
        np.save(os.path.normpath(
            hdf5_name[:-3] + '/clustering_results/electrode {}/clusters{}/bic.npy'.format(electrode_num + 1, i + 3)),
                bic)
            ...
            for feature2 in range(len(data[0])):
                if feature1 < feature2:
                    ...
                    fig.savefig(os.path.normpath(
                        hdf5_name[:-3] + '/Plots/{}/{}_clusters/feature{}vs{}.png'.format(electrode_num + 1, i + 3,
                                                                                          feature2, feature1)))
                    plt.close("all")
        ...
        for ref_cluster in range(i + 3):
            ...
            fig.savefig(os.path.normpath(
                hdf5_name[:-3] + '/Plots/{}/{}_clusters/Mahalonobis_cluster{}.png'.format(electrode_num + 1, i + 3,
                                                                                          ref_cluster)))
            plt.close("all")
        ...
        for cluster in range(i + 3):
            cluster_points = np.where(predictions[:] == cluster)[0]
            fig, ax = AutoSort.Pl2_waveforms_datashader.waveforms_datashader(slices_final[cluster_points, :], x,
                                                                             dir_name=os.path.normpath(hdf5_name[
                                                                                                       :-3] + "_datashader_temp_el" + str(
                                                                                 electrode_num + 1)))
            ...
            fig.savefig(os.path.normpath(
                hdf5_name[:-3] + '/Plots/{}/{}_clusters_waveforms_ISIs/Cluster{}_waveforms'.format(electrode_num + 1,
                                                                                                   i + 3, cluster)))
            plt.close("all")
            ...
            fig.savefig(os.path.normpath(
                hdf5_name[:-3] + '/Plots/{}/{}_clusters_waveforms_ISIs/Cluster{}_ISIs'.format(electrode_num + 1, i + 3,
                                                                                              cluster)))
            plt.close("all")
        ...
        isodf.to_csv(os.path.splitext(hdf5_name)[0] + '/clustering_results/electrode {}/clusters{}/isoinfo.csv'.format(
            electrode_num + 1, i + 3), index=False)
        ...
        for cluster in range(i + 3):
            ...
            cv2.imwrite(hdf5_name[:-3] + '/Plots/{}/{}_clusters_waveforms_ISIs/Cluster{}_Isolation.png'.format(
                electrode_num + 1, i + 3, cluster), isoimg)  # save the image
    with open(hdf5_name[:-3] + '/clustering_results/electrode {}'.format(electrode_num + 1) + '/success.txt',
              'w+') as f:
            ...
            for soln in range(3, maxclust + 1):  # for each number cluster solution
                ...
                for cluster in range(0, soln):  # for each cluster
                    ...
                    text = "Electrode: " + channel + "\nSolution: " + str(soln) + "\nCluster: " + str(
                        cluster)  # text to output to whitespace (cluster, electrode, and solution numbers)
                    cv2_im_rgb = cv2.cvtColor(blank, cv2.COLOR_BGR2RGB)  # convert to color space pillow can use
                    pil_im = Image.fromarray(cv2_im_rgb)  # get pillow image
                    draw = ImageDraw.Draw(pil_im)  # create draw object for text
                    font = ImageFont.truetype(os.path.split(__file__)[0] + "/bin/arial.ttf", 60)  # use arial font
...
def compile_isoi(full_filename, maxclust=7, Lrat_cutoff=.1):
    ...
    for channel in os.listdir(path):  # for each channel
        ...
        try:
            ...
            pass
    with pd.ExcelWriter(os.path.split(path)[0] + '/{}_compiled_isoi.xlsx'.format(os.path.split(path)[-1]),
        ...
        worksheet.conditional_format('A2:H{}'.format(file_isoi.shape[0] + 1),
                                     {'type': 'formula', 'criteria': '=AND($G2>1,$H2>{})'.format(str(Lrat_cutoff)),
                                      'format': redden})
        worksheet.conditional_format('A2:H{}'.format(file_isoi.shape[0] + 1), {'type': 'formula',
                                                                               'criteria': '=OR(AND($G2>.5,$H2>{}),$G2>1)'.format(
                                                                                   str(Lrat_cutoff)),
                                                                               'format': orangen})
        worksheet.conditional_format('A2:H{}'.format(file_isoi.shape[0] + 1),
                                     {'type': 'formula', 'criteria': '=OR($G2>.5,$H2>{})'.format(str(Lrat_cutoff)),
                                      'format': yellen})
        outwrite.save()
