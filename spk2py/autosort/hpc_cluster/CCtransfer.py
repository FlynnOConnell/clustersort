...
# where running logs from the computing hpc_cluster will be placed
...
clustrun = 5  # cutoff for uploading more files to the hpc_cluster


print("Establishing secure connection...")
ssh = SSHClient()
ssh.load_system_host_keys()  # get list of known hosts
ssh.connect('spiedie.binghamton.edu', 22,
            username='dmarshal')  # connect to specified host and account (username).
# Shouldn't need a password argument if key authentification is set up properly
...
for file in finished_files:
    if file.endswith('.h5'):
        ...
        os.system('bash -c "cd /mnt && sudo mount -t drvfs R: r; rsync -r dmarshal@spiedie.binghamton.edu:' + (
                    remote_results_filepath + '/' + os.path.splitext(file)[0]).replace(" ", '_') + ' ' + unix_path(
            results_path).replace(" ", '\ ') + '; sudo umount r"')
        i, o, error = ssh.exec_command(
            "rm -r " + "\"" + (remote_results_filepath + '/' + os.path.splitext(file)[0]).replace(" ", '_') + "\"")
        shutil.move(running_path + '\\' + os.path.splitext(file)[0] + '.pl2',
                    completed_path + '\\' + os.path.splitext(file)[0] + '.pl2')
        sftp.remove(remote_results_filepath + '/' + file)
        shutil.move(running_path + '\\' + os.path.splitext(file)[0] + '.h5',
                    results_path + '\\' + os.path.splitext(file)[0] + '.h5')
...
for file in any_files:  # if there are no pl2 files to run and no h5 files on the hpc_cluster waiting to be run, exit
...
if start_cluster <= clustrun:  # if there are not too many files on the hpc_cluster
    ...
    for file in runfiles:
        ...
        if file.endswith('.pl2'):
            ...
            shutil.move(to_run_path + '\\' + file,
                        running_path + '\\' + file)  # move the file to the folder for currently running files
            with open(os.devnull, 'w') as devnull:  # suppresses function output
                with contextlib.redirect_stdout(devnull):
                    pl2_to_h5(file, running_path,
                              min_licks)  # this will pull data from the pl2 file and package it in an h5 file
            if not os.path.exists((running_path + '\\' + file)[
                                  :-4] + "\\NoCellSortingRunOnThisFile.txt"):  # Determine if the rat licked enough
                print('Uploading preprocessed data for', file[:-4] + '...')
                h5_filepath = os.path.splitext(running_path + '\\' + file)[0] + '.h5'
                os.system(
                    'bash -c "cd /mnt && sudo mount -t drvfs R: r; rsync -r' + ' ' + unix_path(h5_filepath).replace(" ",
                                                                                                                    '\ ') + ' ' + 'dmarshal@spiedie.binghamton.edu:' + (
                        remote_to_run_path) + ';sudo umount r"')
                # queue up a job for the autosort on the remote server
                stdin, stdout, stderr = ssh.exec_command(
                    "cd " + python_script_path + ";module add slurm/current; sbatch start_autosort.sh")
                ...
                shutil.move(running_path + '\\' + os.path.splitext(file)[0],
                            results_path + '\\' + os.path.splitext(file)[0])
