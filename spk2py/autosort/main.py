...
if __name__ == '__main__':
    ...
    # Pull parameters from config file, or create a new file if it is an old version or there is no file
    ...
    if manual_or_auto == 'Auto':
        if datetime.datetime.weekday(datetime.date.today()) == 4:
            ...
            n_files = weekday_n
    elif manual_or_auto == 'Manual':
        n_files = manual_n  # if the script is being run manually, use the manual N
    else:
    ...
    filedates = []
    runfiles = []
    checkfiles = os.listdir(to_run_path)
    iterfiles = checkfiles.copy()
    # We only want .smr files
    for i in range(0, len(iterfiles)):
        if not iterfiles[i].endswith('.smr'):
            checkfiles.remove(iterfiles[i])
            continue
        filedates.append(os.path.getctime(to_run_path + '/' + iterfiles[i]))
    if len(filedates) == 0:
    ...
    for file in runfiles:  # loop through each file
        ...
        AS.pl2_to_h5(file, running_path,
                     min_licks)  # this will pull data from the pl2 file and package it in an h5 file
        if os.path.isfile(running_path + '/' + os.path.splitext(file)[
            0] + "/NoCellSortingRunOnThisFile.txt"):  # If sorting wasnt run, clean everything up
        ...
        hf5 = hdf5.open_file(hdf5_name, 'r')
        if hf5.root.__contains__('/SPKC'):
            elNum = len(hf5.list_nodes("/SPKC"))
        else:
            elNum = int(len(hf5.list_nodes("/SPKwf")) / 2)
        hf5.close()
        ...
        runs = math.ceil(
            elNum / num_cpu)  # Determine how many times to loop through processing based on cpu cores used and number of electrodes
        ...
        reruns = 0
        while len(
                bad_runs) > 0 and reruns < resort_limit:  # while there are still bad runs, and the script has not reached the resort limit
            reruns += 1
            bad_runs = []
