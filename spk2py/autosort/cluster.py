...
def get_filtered_electrode(el, freq=None, sampling_rate=40000.0):
    if freq is None:
        freq = [300, 6000.0]
...
def extract_waveforms(filt_el, spike_snapshot=None, sampling_rate=40000.0, STD=2.0, cutoff_std=10.0):
    ...
    for i in range(len(changes) - 1):
        minimum = np.where(filt_el[pos[changes[i]:changes[i + 1]]] == np.min(filt_el[pos[changes[i]:changes[i + 1]]]))[
            0]
        # print minimum, len(slices), len(changes), len(filt_el) try slicing out the putative waveform, only do this
        # if there are 10ms of data points (waveform is not too close to the start or end of the recording)
        if pos[minimum[0]
                ...
                tempslice = filt_el[
                            pos[minimum[0] + changes[i]] - int((spike_snapshot[0] + 0.1) * (sampling_rate / 1000.0)):
                            pos[minimum[0] + changes[i]] + int((spike_snapshot[1] + 0.1) * (sampling_rate / 1000.0))
                            ]
...
def dejitter(slices, spike_times, spike_snapshot=None, sampling_rate=40000.0):
    ...
    for i in range(len(slices)):
        ...
        if np.abs(minimum - int((spike_snapshot[0] + 0.1) * (sampling_rate / 100.0))) < int(
                10.0 * (sampling_rate / 10000.0)):
...
def clusterGMM(data, n_clusters, n_iter, restarts, threshold):
    ...
    for i in range(restarts):
        g.append(GaussianMixture(n_components=n_clusters, covariance_type='full', tol=threshold, random_state=i,
                                 max_iter=n_iter))
        g[-1].fit(data)
        if g[-1].converged_:
...
def get_Lratios(data, predictions):
    ...
    for ref_cluster in np.sort(np.unique(predictions)):
        ...
        Ls = [1 - chi2.cdf((mahalanobis(data[point, :], ref_mean, ref_covar_I)) ** 2, df) for point in
              np.where(predictions[:] != ref_cluster)[0]]
