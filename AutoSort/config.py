...
def default_config(path):
    ...
    config['Paths'] = {'Pl2 To-Run Path': r'R:\Dannymarsh Sorting Emporium\pl2_to_be_sorted',
                       'Running Path': r'R:\Dannymarsh Sorting Emporium\running_files',
                       'Results Path': r'R:\Dannymarsh Sorting Emporium\Results',
                       'Completed Pl2 Path': r'R:\Dannymarsh Sorting Emporium\completed_pl2',
                       'Use Paths': '1', 'Else Path': ''}
    config['Clustering'] = {'Max Clusters': '7', 'Max Iterations': '1000', 'Convergence Criterion': '.0001',
                            'Random Restarts': '10', 'L-ratio Cutoff': '.1'}
    config['Signal'] = {'Disconnect Voltage': '1500', 'Max Breach Rate': '.2', 'Max Breach Count': '10',
                        'Max Breach Avg.': '20', 'Intra-Cluster Cutoff': '3'}
    ...
    config['Post Process'] = {'reanalyze': '0', 'simple gmm': '1', 'image size': '70',
                              'temporary dir': os.path.expanduser('~') + '\\tmp_python'}
...
def read_config(path):
    ...
    if config_ver != int(params['config version']):
        ...
        default_config(path)
        print(
            'Config version updated, config file reset to default, your original config file has been renamed. Find the new config file here: ' + path)
    else:
        return params
